          ...
          {doubledSkills.map((skill, index) => (
            <div key={index} className="flex flex-col items-center min-w-[70px] sm:min-w-[80px] md:min-w-[100px]">
              <img
                src={skill.icon}
                alt={skill.name}
                className="w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 object-contain hover:scale-x-2 transition-all duration-500"
              />
              <span className="mt-2 sm:mt-3 md:mt-4 text-[14px] sm:text-[16px] md:text-[18px] font-semibold text-black uppercase tracking-widest">
                {skill.name}
              </span>
            </div>
          ))}
